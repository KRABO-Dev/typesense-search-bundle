...
var IsoProductTypesense =
{
	 ...
	 * @param string
	 */
	index: function(el, offset, startTxt, endTxt){
		if(offset==0){
			document.id('refresh-typesense').set('html','<div id="tl_header">'+startTxt+'</div><div id="message"></div>');
		...
		var request = new Request.Contao({
			...
			data: 'isAjax=1&action=typesense_iso_product_index&offset=' + offset + '&REQUEST_TOKEN=' + REQUEST_TOKEN,
			onSuccess: function(content, json){
				if(json.data){
					if (json.data.offset != 'finished'){
						IsoProductTypesense.index(el,json.data.offset);
						document.id('message').set('html',json.data.message);
					}
					else{
						document.id('refresh-typesense').set('html','<div id="tl_header">'+json.data.message+'</div>');
						this.spinner.destroy();
					}
				}
				else{
					IsoProductTypesense.getError(json);
				}
			}.bind(this),
			onError: function(txt, error){
				IsoProductTypesense.getError(error);
			}
		}).send();
	},
	...
	getError: function(error){
		if(error){
			document.id('refresh-typesense').set('html','<div class="tl_error">'+ error +'</div>');
		} else{
			...
			document.id('refresh-typesense').set('html','<div class="tl_error">An Error Occurred</div>');
		}
	},
